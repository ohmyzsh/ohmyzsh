...
if LANG= git pull --quiet --rebase $remote $branch; then
  ...
  if [[ $verbose_mode == default ]]; then
    ...
    printf '\n'
    printf "${BLUE}%s${RESET}\n\n" "$message"
    printf "${BLUE}${BOLD}%s %s${RESET}\n" "To keep up with the latest news and updates, follow us on X:" "$(fmt_link @ohmyzsh https://x.com/ohmyzsh)" "and Bluesky:" "$(fmt_link @ohmyz.sh https://bsky.app/profile/ohmyz.sh)"
